...
ggplot_youdrawit_payload <- function(p) {
  if (!inherits(p, "ggplot")) {
    stop("ggplot_youdrawit_payload() expects a ggplot2 object. Did you make sure to wrap the ggplot in ",
         "parentheses before piping it in?",
         call. = FALSE)
  }

  supported_geoms <- c("point", "line", "smooth") # Update this as more are available

  # Check all geom layers  --------------------------------------
  geom_types <- vapply(p$layers, function(layer) {
    geom_class <- class(layer$geom)[1]
    tolower(sub("^Geom", "", geom_class))
  }, character(1))

  unsupported <- setdiff(geom_types, supported_geoms)
  ...
  if (length(unsupported) > 0) {
    stop(
      paste0(
        ...
        ". Supported geoms are: ",
        paste(supported_geoms, collapse = ", "),
        "."
      ),
      call. = FALSE
    )
